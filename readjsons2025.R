...
tall<-abs(max(ywid-jdf[which(jdf$joint==0),4],na.rm=T)-min(ywid-jdf[which(jdf$joint==21),4],na.rm=T))
#smr<-data.frame(matrix(NA,length(jofi),9))
smr<-data.frame()
for (i in 1:length(jofi)){
  smr[i,1]<-jntn[i]
  smr[i,2:5]<-c(min(jdf[which(jdf$joint==jofi[i]),3],na.rm=T),max(jdf[which(jdf$joint==jofi[i]),3],na.rm=T),min(ywid-jdf[which(jdf$joint==jofi[i]),4],na.rm=T),max(ywid-jdf[which(jdf$joint==jofi[i]),4],na.rm=T))
}
